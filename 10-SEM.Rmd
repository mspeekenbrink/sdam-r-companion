...
We will just cover the very basics of Exploratory Factor Analysis (EFA) here. In EFA, all "factors" (which are the latent variables in the model), are related to all observed variables (sometimes also called the "manifest" variables). 

EFA is implemented in the `factanal()` function from the `stats` package in base R, and the `fa()` function in the `psych` package. We will focus on the latter package here. The `psych` package comes with various additional tools useful for EFA, and adheres to the conventions for EFA in psychology. 

### Determining the number of factors with Parallel Analysis

A first useful function implements so-called "parallel analysis" to determine the optimal number of underlying factors for a given set of observed variables. This method, whilst somewhat "hacky", tends to work well in practice.
...
```{r}
semPlot::semPaths(bfi_cfa, layout="tree", sizeMan=3, sizeInt = 5, style="ram", intercepts=FALSE, residuals=TRUE, rotation=1, intAtSide = FALSE, whatLabels = "est", nCharNodes = 5, normalize=TRUE, width=12, height=6)
```

If you want to obtain a solution for uncorrelated factors, you need to explicitly fix the covariances between the factors to 0. This can be done via the `~~` operator, and multiplying the factors on the right-hand side of this operator to 0. For example, the line
```
...
```{r}
semPlot::semPaths(bfi_cfa2, layout="tree", sizeMan=3, sizeInt = 5, style="ram", intercepts=FALSE, residuals=TRUE, rotation=1, intAtSide = FALSE, whatLabels = "est", nCharNodes = 5, normalize=TRUE, width=12, height=6)
```
...
```{r}
lavaan::modificationIndices(bfi_cfa2) 
```

`lavaan::modificationIndices(, minumum.value = 10)`

```{r}
lavaan::modificationIndices(bfi_cfa2, minumum.value = 10) 
```

`lavaan::modificationIndices(, minumum.value = 10, order = TRUE)`

```{r}
lavaan::modificationIndices(bfi_cfa2, minumum.value = 10, order=TRUE) 
```
