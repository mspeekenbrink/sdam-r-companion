...
by the authors. Se let's first clean it up a little:
```{r}
dat <- cheerleader
...
Relabelling factors is somewhat tedious with base R. It's easier to use the `fct_recode` function from the `forcats` package. 
...
Here, we will use the `pivot_longer` and `pivot_wider` functions from the `tidyr` 
package. 
...
which columns in the wide formate to transform into a single variable
...
within-subjects structure through the `idata` argument, as well as a formula in the 
`idesign` argument specifying which effects to include as within-subjects factors.
...
```{r}
afex::aov_car(Response ~ Version*Presentation + Error(Participant/Presentation), data=ldat)
```