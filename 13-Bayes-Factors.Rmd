...
The specification of the `hypothesis` argument is rather flexible. 
...
```{r}
brms::bridge_sampler(mod_regression)
```
...
mod_priors_alt <- c(# intercept
                prior(normal(0, 100), class = Intercept), 
                # slopes
                prior(normal(0, .0000001), class = b, coef = "hate_groups_per_million"),
                prior(normal(0, 5), class = b, coef = "percent_bachelors_degree_or_higher"),
                # error SD
                prior(normal(0, 10), class = sigma))
