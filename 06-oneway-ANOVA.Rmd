...
## Computing contrast-coded predictors
...
The procedure is easily extended to multiple contrast-coded predictors. For an example, let's consider the full dataset with all four conditions.
...
A nice thing about the `lm` function is that you can also supply factors as predictors directly. Internally, the `lm` function will then create the necessary contrast-coded predictors from the contrasts supplied to the factor. Let's try this:
```{r}
